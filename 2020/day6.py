...
def first_part():
    current_form, sum_yes = [], 0
    for line in input_string.split("\n"):
        if line != "":
            current_form.extend([letter for letter in line if letter not in current_form])
        else:
            sum_yes, current_form = sum_yes + len(current_form), []
    return sum_yes + len(current_form)


def second_part():
    sum_yes, new_group, current_group = 0, True, []
    for line in input_string.split("\n"):
        if line != "":
            if new_group:
                current_group, new_group = [letter for letter in line], False
            else:
                current_group = [letter for letter in current_group if letter in line]
        else:
            sum_yes, new_group = sum_yes + len(current_group), True
    return sum_yes + len(current_group)
